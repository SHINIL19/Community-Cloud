...
CSV_FILE = os.path.join(".", "iot_data.csv")  # Safer
...
def on_message(client, userdata, msg):
    try:
        ...
        # Reshape for model (EllipticEnvelope expects 2D input)
        X = np.array([[temp, hum]])
        
        prediction = model.predict(X)[0]  # 1 means normal, -1 means anomaly
